...
- Trong trang market dang giao dich hoac trang Profile, nguoi dung co the lay quote LI.FI va tu ky swap `USDC <-> EURC` tren Arc Testnet neu can dung token settlement cua market truoc khi stake. Swap khong thay doi token tra thuong cua market.
...
- Nut swap trong trading panel va Profile chi la tien ich cho vi nguoi dung doi USDC/EURC truoc giao dich; route va so nhan uoc tinh den tu LI.FI, giao dich swap duoc ky trong vi.
